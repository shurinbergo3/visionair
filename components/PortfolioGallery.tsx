...
// Picks the matching responsive variant generated by scripts/optimize-images.mjs.
// portfolio-XX.webp → portfolio-XX@mobile.webp / portfolio-XX@desktop.webp.
// Falls back to the original src when the pattern doesn't match.
function buildSrc(src: string, variant: 'mobile' | 'desktop'): string {
  const m = src.match(/^(.*\/portfolio-\d+)\.webp$/);
  return m ? `${m[1]}@${variant}.webp` : src;
}
    ...
    <>
      ...
      >
        {items.map((p, i) => {
          ...
          // Only the very first card is eager — it sits above the fold and is
          // the LCP candidate. Everything else is lazy: the browser will fetch
          // them as they scroll into view. Once the user expands the gallery
          // on mobile, the newly revealed cards still get a fresh mount (via
          // `key`) so iOS Safari doesn't keep the original `display:none`
          // decision and leave them blank.
          const eager = i === 0;
          ...
          return (
            ...
            >
              ...
              <img
                src={buildSrc(p.src, 'desktop')}
                srcSet={`${buildSrc(p.src, 'mobile')} 960w, ${buildSrc(p.src, 'desktop')} 1600w`}
                sizes="(max-width: 640px) 100vw, (max-width: 1080px) 50vw, 33vw"
                alt={p.alt}
                loading={eager ? 'eager' : 'lazy'}
                decoding="async"
                fetchPriority={eager ? 'high' : 'auto'}
                width={orient === 'portrait' ? 900 : 1600}
                height={orient === 'portrait' ? 1600 : 900}
              />
      ...
      {current && openIdx !== null && (
        ...
        >
          ...
          <figure className="lbx-frame">
            ...
            <img
              key={current.src}
              src={buildSrc(current.src, 'desktop')}
              alt={current.alt}
              className="lbx-img"
            />
